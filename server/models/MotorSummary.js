...
/**
 * Motor Summary Models - User-Based ML Training Records
 * 
 * AURA version: Uses userId instead of sessionId
 * EXACT COPY of sensecheck's MotorSummary models
 * 
 * These collections store derived features ready for ML training.
 * NOT bucketed (one document per round/user).
...
motorRoundSummarySchema.index({ userId: 1, round: 1 }, { unique: true });

// Index for ML training queries
motorRoundSummarySchema.index({ userId: 1, createdAt: 1 });
...
const MotorRoundSummary = mongoose.model('MotorRoundSummary', motorRoundSummarySchema);

// ===== MotorSessionSummary =====
// Stores overall user features and labels
...
// Index for ML training queries
motorSessionSummarySchema.index({ userId: 1, createdAt: 1 });
...
const MotorSessionSummary = mongoose.model('MotorSessionSummary', motorSessionSummarySchema);

// Helper function to compute features from attempts
async function computeRoundFeatures(userId, round) {
  const MotorAttemptBucket = mongoose.model('MotorAttemptBucket');
  const attempts = await MotorAttemptBucket.getUserAttempts(userId, round);
...
async function computeSessionFeatures(userId) {
  const features1 = await computeRoundFeatures(userId, 1);
  const features2 = await computeRoundFeatures(userId, 2);
  const features3 = await computeRoundFeatures(userId, 3);
