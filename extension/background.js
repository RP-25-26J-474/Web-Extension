...
async function syncProfileBeforeLogout(userId) {
  const mlFeedbackUrl = API_CONFIG.ML_SESSION_FEEDBACK_URL || 'https://api-gateway.auraui.org/api/ml-engine/user/trigger-update';
  try {
...
async function fetchMlPersonalizedProfile() {
  try {
    ...
    const baseUrl = API_CONFIG.ML_PROFILE_API_URL || 'https://api-gateway.auraui.org/api/ml-engine/data/current-profile';
...
async function fetchInitialMlProfileFromImpairment() {
  try {
    ...
    const url = API_CONFIG.IMPAIRMENT_TO_ML_PROFILE_API_URL || 'https://api-gateway.auraui.org/api/ml-engine/category/generate-profile';
